...
- Median Exit Velocity -> `median_ev` (needs new data)
- Exit Velocity Percentiles (10th, 50th, 90th) -> `ev_p10`, `ev_p50`, `ev_p90` (needs new data)
...
- Barrels per Ball in Play -> `barrels_per_bip` (needs new data)
- Sweet-Spot % -> `sweet_spot_pct` (needs new data)
- Average Launch Angle -> `la` (available)
- Launch Angle Standard Deviation -> `la_sd` (needs new data)
...
- Take % -> `take_pct` (needs new data)
- Take in Zone % -> `take_in_zone_pct` (needs new data)
- Take out of Zone % -> `take_out_zone_pct` (needs new data)
- First-Pitch Swing % -> `first_pitch_swing_pct` (needs new data)
- First-Pitch Take % -> `first_pitch_take_pct` (needs new data)
- Two-Strike Swing % -> `two_strike_swing_pct` (needs new data)
- Two-Strike Whiff % -> `two_strike_whiff_pct` (needs new data)

### Contact Quality Buckets
- Under % -> `under_pct` (needs new data)
- Topped % -> `ttopct` (available)
- Flare/Burner % -> `flare_burner_pct` (needs new data)
- Solid Contact % -> `hardpct` (available)
- Weak Contact % -> `softpct` (available)
- Poorly Hit % -> `poorly_hit_pct` (needs new data)
- Poorly Under % -> `poorly_under_pct` (needs new data)
- Poorly Topped % -> `poorly_topped_pct` (needs new data)
- Poorly Weak % -> `poorly_weak_pct` (needs new data)
