...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Get bottom N least valuable players by TVP."
    )
    parser.add_argument(
        "--count",
        type=int,
        default=25,
        help="Number of bottom players to return (default: 25).",
    )
    ...
    sorted_players = sorted(valid_players, key=lambda x: x["tvp_mlb"])
    bottom_players = sorted_players[: args.count]

    for rank, player in enumerate(bottom_players, 1):
